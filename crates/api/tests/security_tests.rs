...
fn setup_api() -> (KillOnDrop, String) {
    ...
    client.post("http://127.0.0.1:3000/auth/register")
        .json(&json!({
            "email": email,
    ...
    let login_res = client.post("http://127.0.0.1:3000/auth/login")
        .json(&json!({
            "email": email,
            "password": "password123"
        }))
    ...
    let token = login_data["token"].as_str().expect("Token not found").to_string();
...
fn test_security_xss_filename() {
    ...
    let form = multipart::Form::new()
        .part("file", multipart::Part::text("Date,Time,Tag ID\n2024-01-01,10:00,123")
            .file_name(xss_filename));

    let res = client.post("http://127.0.0.1:3000/api/analyze")
        .bearer_auth(&token)
        .multipart(form)
        .send()
    ...
    let history_res = client.get("http://127.0.0.1:3000/api/history")
        .bearer_auth(&token)
        .send()
...
fn test_security_binary_content_rejection() {
    ...
    let form = multipart::Form::new()
        .part("file", multipart::Part::bytes(binary_data)
...
fn test_security_large_file_handling() {
    ...
    let form = multipart::Form::new()
        .part("file", multipart::Part::text(large_data)
            .file_name("large.csv"));
    ...
    assert!(res.status().is_success(), "Expected success for 5MB file, got: {:?}", res.status());
}
