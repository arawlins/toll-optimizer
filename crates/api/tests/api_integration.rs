...
fn test_registration_and_login() {
    ...
    let register_res = client.post("http://127.0.0.1:3000/auth/register")
    ...
    assert!(register_res.status().is_success(), "Registration failed: {:?}", register_res.text());
    ...
    assert!(login_res.status().is_success(), "Login failed: {:?}", login_res.text());
    
    let login_data: serde_json::Value = login_res.json().expect("Failed to parse login JSON");
    assert!(login_data["token"].is_string(), "Token not found in login response");
}
